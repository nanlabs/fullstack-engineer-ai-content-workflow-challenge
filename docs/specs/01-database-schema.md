...
- [ ] `docker compose up` brings up postgres with the `acme_content` DB.
- [ ] `uv run alembic upgrade head` runs cleanly with no errors.
- [ ] Connect to the DB and `\dt` shows all tables.
- [ ] Connect to the DB and `\dT` shows the enums.
- [ ] `uv run python -m scripts.seed` leaves the DB with the demo campaign.
- [ ] `SELECT * FROM campaign;` returns the campaign.
- [ ] Tests:
